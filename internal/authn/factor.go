...
type Factor struct {
	...
	// Use one of the FactorType* constants below; foreign strings are
	// preserved across [Aggregate] but contribute no amr value.
	//
	...
	// reads it for [FactorTypePasskey], where UV distinguishes the
	// "swk" and "hwk" RFC 8176 tokens; other methods MAY leave it
	// false.
	UserVerified bool
}

// Pre-defined factor type identifiers. The string values are stable:
// they double as the AMR identifiers in [op/interaction.Result.AMR] and
// as the values written to log lines and audit records.
const (
	// FactorTypePassword denotes a knowledge factor (a shared secret
	// the user typed). RFC 8176 amr value: "pwd".
	FactorTypePassword = "password"

	// FactorTypeTOTP denotes RFC 6238 one-time codes generated by an
	// authenticator app. RFC 8176 amr value: "otp".
	FactorTypeTOTP = "totp"

	// FactorTypeRecoveryCode denotes a single-use recovery code the
	// user pre-stored at enrolment. RFC 8176 amr value: "otp" (see
	// [Factor.AMRValue] for the rationale).
	FactorTypeRecoveryCode = "recovery_code"

	// FactorTypePasskey denotes a WebAuthn assertion. RFC 8176 amr
	// value depends on [Factor.UserVerified]: "hwk" when UV is set,
	// "swk" otherwise.
	FactorTypePasskey = "passkey"
)
...
//   - "otp" — one-time password, both for TOTP and for recovery
//     codes. Recovery codes are pre-issued numeric codes the user
...
func (f Factor) AMRValue() string {
	...
	case FactorTypePassword:
		return "pwd"
	case FactorTypeTOTP, FactorTypeRecoveryCode:
		return "otp"
	case FactorTypePasskey:
		if f.UserVerified {
			return "hwk"
		}
		return "swk"
