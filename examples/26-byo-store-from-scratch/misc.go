...
// Delete removes the scratch row and reports whether what it removed was
// live. The DELETE statement alone cannot tell a live row from an expired
// one still awaiting collection, so the read comes first: an expired
// interaction is absent on every path, and the row is reclaimed regardless.
