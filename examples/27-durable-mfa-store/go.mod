// Module github.com/libraz/go-oidc-provider/examples/27-durable-mfa-store
// pairs the SQL storage adapter with a hand-written, persistent
// store.TOTPStore that lives in the same *sql.DB as the core tables. It
// is its own sub-module so the SQL driver dependencies (modernc.org/sqlite)
// and the RP-side dependencies (golang.org/x/oauth2,
