...
// host. The library enforces the OIDC Core §5 "https only" rule for
...
// uniqueHosts collapses redirectURIs to their distinct host values,
// preserving order so the function is deterministic for tests.
// Malformed URIs and entries without a host are skipped silently;
// the caller decides how to surface the empty result.
