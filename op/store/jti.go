...
// arrives later.
//
// ConsumedJTIStore is explicitly NOT part of the transactional cluster. The
...
type ConsumedJTIStore interface {
	...
	// if the same jti has already been marked, even when the previous
	// expiresAt has not yet been reached. expiresAt is the wall-clock
	// time at which the record may safely be discarded; backends use it
	// to drive TTL-based eviction. The caller is responsible for
	// computing expiresAt from the JWT's exp claim.
