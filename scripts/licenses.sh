...
run_module() {
  ...
  local tags="${2:-}"
  if [ -n "$tags" ]; then
    log "go-licenses report ./... ($mod, tags=$tags)"
    ( cd "$mod" && GOFLAGS="-tags=$tags" "$LICENSES" report ./... 2>/dev/null ) >> "$MERGED"
  else
    log "go-licenses report ./... ($mod)"
    ( cd "$mod" && "$LICENSES" report ./... 2>/dev/null ) >> "$MERGED"
  fi
}
...
INDEX="$REPO_ROOT/THIRD_PARTY.md"
{
  ...
  printf 'This file is generated by `scripts/licenses.sh` from `go-licenses report ./...` runs '
