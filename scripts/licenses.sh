...
# Regenerate THIRD_PARTY.md from the same report so the committed
# index never drifts from go.mod. The project's own module is
# filtered out (the repo's LICENSE is the canonical source for its
# own terms); rows are sorted lexicographically for a stable diff.
